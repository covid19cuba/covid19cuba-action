...
def generate(debug=False):
    ...
    dump(
        ...
        open("api/v1/all.json", mode="w", encoding="utf-8"),
        ensure_ascii=False,
        ...
        for item in diagnosed:
            if item.get("sexo") is None:
                ...
                try:
                    ...
                    result[item.get("sexo")] = 1
    pretty = {"hombre": "Hombres", "mujer": "Mujeres", "no reportado": "No Reportados"}
    hard = {"hombre": "men", "mujer": "women", "no reportado": "unknown"}
    return {
        hard[key]
        if key in hard
        else key: {
...
def evolution_of_cases_by_days(data):
    ...
    for x in days:
        ...
        date.append(x["fecha"])
    return {
        "accumulated": {"name": "Casos acumulados", "values": accumulated[1:]},
        "daily": {"name": "Casos en el día", "values": daily[1:]},
        "active": {"name": "Casos activos", "values": actives},
        "date": {"name": "Fecha", "values": date},
    }
...
def evolution_of_deaths_by_days(data):
    ...
    for x in days:
        ...
        date.append(x["fecha"])
    return {
        "accumulated": {"name": "Fallecimientos acumulados", "values": accumulated[1:]},
        "daily": {"name": "Fallecimientos en el día", "values": daily[1:]},
        "date": {"name": "Fecha", "values": date},
    }
...
def evolution_of_recovered_by_days(data):
    ...
    for x in days:
        ...
        date.append(x["fecha"])
    return {
        "accumulated": {"name": "Altas acumuladas", "values": accumulated[1:]},
        "daily": {"name": "Altas en el día", "values": daily[1:]},
        "date": {"name": "Fecha", "values": date},
    }
...
def cases_by_nationality(data):
    pretty = {"foreign": "Extranjeros", "cubans": "Cubanos", "unknown": "No reportados"}
...
def list_of_tests_performed(data):
    ...
    return {
        "positive": {"name": "Tests Positivos", "value": positive},
        "negative": {"name": "Tests Negativos", "value": total - positive},
        "total": {"name": "Total de Tests", "value": total},
    }
...
def tests_by_days(data):
    ...
    for day in (x for x in days if "diagnosticados" in x):
        ...
        if "tests_total" in day:
            ...
            prev_test_positive = total
    return {
        "date": {"name": "Fecha", "values": ntest_days[1:]},
        "negative": {"name": "Tests Negativos", "values": ntest_negative[1:]},
        "positive": {"name": "Tests Positivos", "values": ntest_positive[1:]},
        "total": {"name": "Total de Tests", "values": ntest_cases[1:]},
    }
...
def tests_positive_percent(data):
    ...
    return {
        "date": date,
        "daily": {"name": "% de Tests Positivos en el Día", "values": daily},
        "accumulated": {"name": "% de Tests Positivos Acumulados", "values": accum},
    }
...
def map_data(data):
    ...
    return {
        "muns": muns,
        "pros": pros,
        "genInfo": {"max_muns": max_muns, "max_pros": max_pros, "total": total},
    }
...
def curves_evolution(data):
    ...
    return {
        trans_countries[i]: j
        for i, j in (
            list(sorted(curves.items(), key=lambda x: x[1]["ctotal"], reverse=True))[
                :ntop
            ]
            + [("Cuba", curves["Cuba"])]
        )
...
def effective_reproductive_number(data):
    ...
    data_cu["dates"] = dates
    return {
        "upper": {"name": "Margen Superior", "values": data_cu["upper"]},
        "value": {"name": "Número Reproductivo Efectivo", "values": data_cu["value"]},
        "lower": {"name": "Margen Inferior", "values": data_cu["lower"]},
        "date": {"name": "Fecha", "values": data_cu["dates"]},
    }
...
def distribution_of_cases(data):
    ...
    return {
        "recovered": {"name": "Recuperados", "value": recovered},
        "active": {"name": "Activos", "value": active},
        "evacuees": {"name": "Evacuados", "value": evacuees},
        "deaths": {"name": "Fallecidos", "value": deaths},
        "cases": {"name": "Casos", "value": cases},
    }
...
def evolution_of_cases_and_recovered_by_days(data):
    ...
    for x in days:
        ...
        date.append(x["fecha"])
    return {
        "diagnosed": {"name": "Casos en el día", "values": diagnosed},
        "recovered": {"name": "Altas en el día", "values": recovered},
        "date": {"name": "Fecha", "values": date},
    }
...
def evolution_of_active_and_recovered_accumulated(data):
    ...
    return {
        "active": {"name": "Casos activos", "values": actives},
        "recovered": {"name": "Altas acumuladas", "values": recovered[1:]},
        "date": {"name": "Fecha", "values": date},
    }
...
def radar_chart_data(data):
    ...
    return {
        "data": radar,
        "bounds": {"stringency": 100, "deaths_p": 15, "recovered_p": 100, "test_p": 40},
    }
