...
def generate(debug=False):
    ...
    for key in province_abbrs:
        ...
        municipalities = [
            (key, key.split(".")[1])
            for key in municipality_codes
            if key.startswith(dpa_code)
        ]
            ...
            dump(
                {
                    f.__name__: dump_util(
                        f"api/v1/provinces/{key}/municipalities/{full_code}",
                        f,
                        data_cuba=data_cuba,
                        province=value,
                        debug=debug,
                ...
                open(
                    f"api/v1/provinces/{key}/municipalities/{full_code}/all.json",
                    mode="w",
                    encoding="utf-8",
                ),
                ensure_ascii=False,
...
def resume(data):
        ...
        len(
            list(
                filter(
                    ...
                    == data["province"]
                    and a.get("municipio_detección")  # type: ignore
                    == data["municipality"],
    ...
    result = [{"name": "Diagnosticados", "value": diagnosed}]
    if diagnosed:
...
def cases_by_sex(data):
    ...
    for diagnosed in (x["diagnosticados"] for x in days if "diagnosticados" in x):
        for item in diagnosed:
            if (
                item.get("provincia_detección") != data["province"]
                or item.get("municipio_detección") != data["municipality"]
            ):
                continue
