...
def generate(debug=False):
    ...
    for key in province_abbrs:
        ...
        dump(
            ...
            open(f"api/v1/provinces/{key}/all.json", mode="w", encoding="utf-8"),
            ensure_ascii=False,
...
def map_data(data):
    ...
    for key in muns:
        ...
        if key:
            total += muns[key]
    return {"muns": muns, "genInfo": {"max_muns": max_muns, "total": total}}
    ...
    for patients in diagnosed:
        for p in patients:
            ...
            try:
                ...
                counter[p[dpacode]] = {"value": 1, "name": p["municipio_detección"]}
            total += 1
    result = []
...
def effective_reproductive_number(data):
    ...
    return {
        "upper": {"name": "Margen Superior", "values": data_prov["upper"]},
        "value": {"name": "Número Reproductivo Efectivo", "values": data_prov["value"]},
        "lower": {"name": "Margen Inferior", "values": data_prov["lower"]},
        "date": {"name": "Fecha", "values": data_prov["dates"]},
    }
